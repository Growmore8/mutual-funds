<x-client-layout title="Spot Trading">
    ...
    <div x-data="spot()" x-init="init()" class="-mx-1">
        {{-- Spot account summary — both wallets, SEPARATE from the mutual-fund pool --}}
        <div class="gcard rounded-2xl p-4 mb-3 mx-1 bg-white dark:bg-white/[0.04]">
            ...
            <p class="text-[10px] text-gray-400 mt-1">Not linked to your Mutual Fund pool.</p>
        </div>
        ...
        {{-- Symbol header --}}
        <div class="flex items-center justify-between px-1 mb-3" x-data="{ pick:false, flt:'{{ $selected->market ?? 'india' }}' }">
            <div class="relative">
                ...
                <div x-show="pick" @click.outside="pick=false" x-cloak class="absolute z-30 mt-1 w-72 bg-white dark:bg-[#0a1730] border border-gray-200 dark:border-white/10 rounded-xl shadow-xl p-2">
                    ...
                    <div class="max-h-64 overflow-y-auto">
                        @foreach ($instruments as $m)
                            <a href="{{ route('spot.index', ['symbol' => $m->symbol]) }}" x-show="flt==='{{ $m->market }}'"
                               class="flex justify-between px-3 py-2 rounded-lg text-sm hover:bg-gray-50 dark:hover:bg-white/5 {{ $selected && $selected->id===$m->id ? 'bg-gray-100 dark:bg-white/10' : '' }}">
        ...
        <div class="grid grid-cols-2 gap-3 px-1">
            <div>
                ...
                <input x-show="otype==='limit'" x-model="oprice" type="number" placeholder="Price ({{ $cs }})"
                ...
                <template x-if="side==='buy'">
                    <input x-model="ototal" type="number" placeholder="Total ({{ $selected->currency ?? 'USD' }})" class="w-full bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-lg px-3 py-2.5 text-sm mb-2">
                </template>
    ...
    <script>
        function spot(){
            return {
                ...
                init(){ if(!this.id) return; this.tick(); this._t=setInterval(()=>this.tick(), 6000); this.$watch('showChart', v=>{ if(v) this.loadCandles(); }); },
