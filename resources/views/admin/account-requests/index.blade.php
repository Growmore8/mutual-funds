<x-admin-layout title="Requests · Account Requests">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-5">
        <form method="GET" class="flex gap-2 text-sm">
            ...
            @if (!empty($search))<a href="{{ route('admin.account-requests.index', array_filter(['status' => request('status')])) }}" class="px-4 py-2 border rounded-md text-gray-600">Clear</a>@endif
        </form>
        <div class="flex gap-1 text-sm">
            @foreach (['' => 'All', 'pending' => 'Pending', 'approved' => 'Approved', 'rejected' => 'Rejected'] as $key => $label)
                <a href="{{ route('admin.account-requests.index', array_filter(['status' => $key, 'q' => $search ?? ''])) }}"
