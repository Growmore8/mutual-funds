    ...
    <div class="bg-white shadow rounded-xl overflow-hidden">
        <table class="min-w-full text-sm">
            <thead class="bg-gray-50 text-gray-500 text-left">
                <tr><th class="px-4 py-3">Client</th><th class="px-4 py-3">Type</th><th class="px-4 py-3">Doc #</th><th class="px-4 py-3">Submitted</th><th class="px-4 py-3 text-right">Actions</th></tr>
            </thead>
            <tbody class="divide-y divide-gray-100">
                @forelse ($documents as $d)
                    <tr>
                        ...
                        <td class="px-4 py-3">{{ ucwords(str_replace('_',' ',$d->doc_type)) }}</td>
                        <td class="px-4 py-3">{{ $d->document_number ?? '—' }}</td>
                        <td class="px-4 py-3 text-gray-400">{{ $d->created_at->diffForHumans() }}</td>
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.kyc.file',$d) }}" target="_blank" class="px-3 py-1.5 bg-gray-100 rounded-md">View file</a>
