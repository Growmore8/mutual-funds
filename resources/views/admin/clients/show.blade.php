<x-admin-layout title="Client · {{ $client->name }}">
    @php
        ...
        $cpnl = $client->runningPnl();
        $mfCapital = (float) $client->totalDeposited();
        $usBal = (float) $spotUsd->balance;
    @endphp

    {{-- Header: who + quick status --}}
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.clients.index') }}" class="text-gray-400 hover:text-gray-600"><i class="fa-solid fa-arrow-left"></i></a>
            <div>
                <h1 class="text-lg font-bold text-gray-900">{{ $client->name }}
        ...
        <div class="flex items-center gap-2 text-xs">
            <span class="px-2.5 py-1 rounded-full {{ ['pending'=>'bg-gray-100 text-gray-600','active'=>'bg-green-100 text-green-800','suspended'=>'bg-red-100 text-red-800','locked'=>'bg-amber-100 text-amber-800'][$client->status] ?? 'bg-gray-100' }}">{{ ucfirst($client->status) }}</span>
            ...
            <span class="px-2.5 py-1 rounded-full {{ $kc }}">KYC: {{ ucfirst(str_replace('_',' ',$client->kyc_status)) }}</span>
        </div>
    </div>
    ...
    <div x-data="{ tab: window.location.hash === '#spot' ? 'spot' : 'fund' }">
        ...
        <div class="flex flex-wrap items-center gap-2 mt-6 mb-4">
            @php $tabBtn = "px-4 py-2 rounded-lg text-sm font-semibold whitespace-nowrap"; @endphp
            <button @click="tab='fund'"     :class="tab==='fund'     ? 'bg-emerald-600 text-white shadow' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'" class="{{ $tabBtn }}"><i class="fa-solid fa-layer-group mr-1"></i> Mutual Fund</button>
            <button @click="tab='spot'"     :class="tab==='spot'     ? 'bg-blue-600 text-white shadow'    : 'bg-gray-100 text-gray-600 hover:bg-gray-200'" class="{{ $tabBtn }}"><i class="fa-solid fa-arrow-trend-up mr-1"></i> Spot Trading</button>
            <button @click="tab='kyc'"      :class="tab==='kyc'      ? 'bg-gray-800 text-white shadow'    : 'bg-gray-100 text-gray-600 hover:bg-gray-200'" class="{{ $tabBtn }}"><i class="fa-solid fa-id-card mr-1"></i> KYC</button>
            <button @click="tab='activity'" :class="tab==='activity' ? 'bg-gray-800 text-white shadow'    : 'bg-gray-100 text-gray-600 hover:bg-gray-200'" class="{{ $tabBtn }}"><i class="fa-solid fa-receipt mr-1"></i> Activity</button>
            <button @click="tab='edit'"     :class="tab==='edit'     ? 'bg-gray-800 text-white shadow'    : 'bg-gray-100 text-gray-600 hover:bg-gray-200'" class="{{ $tabBtn }}"><i class="fa-solid fa-gear mr-1"></i> Edit</button>
        </div>
                        ...
                        <div class="border border-gray-200 rounded-xl p-4">
                            ...
                            <form method="POST" action="{{ route('admin.clients.account.update', [$client, $acc]) }}" class="grid grid-cols-2 gap-2 text-sm">
                                ...
                                <div class="col-span-2 flex flex-wrap gap-4 pt-1">
                                    <label class="flex items-center gap-2 text-xs"><input type="checkbox" name="plan_locked" value="1" @checked($acc->plan_locked) class="rounded"> Lock plan/pool</label>
                                    <label class="flex items-center gap-2 text-xs"><input type="checkbox" name="locked" value="1" @checked($acc->locked) class="rounded text-amber-600"> Lock account (view-only)</label>
                                    <label class="flex items-center gap-2 text-xs"><input type="checkbox" name="active" value="1" @checked($acc->active) class="rounded text-emerald-600"> Active (uncheck = deactivate)</label>
                                </div>
                                <div class="col-span-2 flex items-center justify-between gap-2">
                                    ...
                                    @if ($client->fundAccounts->count() > 1)
                                        <button form="del-acc-{{ $acc->id }}" class="text-xs text-red-600 hover:underline"><i class="fa-solid fa-trash mr-1"></i> Delete account</button>
                                    @endif
                                </div>
                            </form>
                            @if ($client->fundAccounts->count() > 1)
                                <form id="del-acc-{{ $acc->id }}" method="POST" action="{{ route('admin.clients.account.destroy', [$client, $acc]) }}" onsubmit="return confirm('Delete {{ $acc->label }} ({{ $acc->code() }}) and all its records? This cannot be undone.')">@csrf @method('DELETE')</form>
                            @endif
                        </div>
                    @empty
        ...
        <div x-show="tab==='spot'" x-cloak id="spot">
            <div class="bg-white shadow rounded-xl p-6">
                ...
                <div class="grid grid-cols-2 gap-3 mb-4">
                    <div class="rounded-lg border border-gray-200 p-3"><p class="text-xs text-gray-500">USD wallet</p><p class="text-xl font-bold text-gray-900">${{ number_format($usBal,2) }}</p></div>
                ...
                {{-- Adjust balance --}}
                <p class="text-xs font-semibold text-gray-500 mb-1">Adjust wallet</p>
                ...
                <p class="text-[11px] text-gray-400 mb-4">Tip: to fund from an INR amount, use Transactions → Add transaction → “enter INR”.</p>

                {{-- Spot access --}}
                <p class="text-xs font-semibold text-gray-500 mb-1">Access</p>
                <form method="POST" action="{{ route('admin.spot.access', $client) }}" class="flex flex-wrap items-center gap-4 mb-5 p-3 rounded-lg bg-gray-50 border border-gray-200 text-sm">
                    @csrf
                    ...
                    <div class="flex items-center justify-between py-2 border-b border-gray-100 last:border-0 text-sm">
                        <div>
                            ...
                            <div class="mt-1 flex gap-3">
                                @if ($doc->front_path ?? $doc->file_path)<a href="{{ route('admin.kyc.file', [$doc, 'front']) }}" target="_blank" class="text-emerald-600 hover:underline"><i class="fa-regular fa-image"></i> Front</a>@endif
                                @if ($doc->back_path)<a href="{{ route('admin.kyc.file', [$doc, 'back']) }}" target="_blank" class="text-emerald-600 hover:underline"><i class="fa-regular fa-image"></i> Back</a>@endif
                            </div>
                        </div>
        ...
        <div x-show="tab==='edit'" x-cloak>
            <div class="bg-white shadow rounded-xl p-6 max-w-2xl">
                ...
                <form method="POST" action="{{ route('admin.clients.destroy', $client) }}" class="mt-4 pt-4 border-t border-gray-100" onsubmit="return confirm('Delete this client and all their data? This cannot be undone.')">
                    @csrf @method('DELETE')
                    <button class="px-4 py-2 bg-red-600 text-white rounded-md text-sm"><i class="fa-solid fa-trash mr-1"></i> Delete client</button>
                </form>
            </div>
        </div>
    </div>
